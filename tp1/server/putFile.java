...
public class putFile implements Runnable{
    ...
    public putFile(UDPPacket connectionPacket, String fileName) {
    ...
    public void start() {
        try {
            ...
            firstPacket =1;
            UDPPacket confirmPacket = new UDPPacket(connectionPacket.getRequest(),connectionSocket.getInetAddress(),connectionSocket.getPort(),connectionPacket.getFromAddress(),connectionPacket.getFromPort(), 1, ack, lastPacket, new byte[1024]);
            send(confirmPacket);
    ...
    private synchronized void readFile(){
        ...
        try {
            ...
            reader.skip(seq-1);
            while(reader.read(buffer,0,buffer.length) != -1 && packetList.size() < 5  ){
